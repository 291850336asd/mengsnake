...
public class NormalGameEngine {
    ...
    public ConcurrentHashMap<String, SnakeEntity> snakes = new ConcurrentHashMap<>();
    ...
    public NormalGameEngine(String uuid){
       this(300, 300,uuid);
    }
        ...
        mapFutrue = executorService.scheduleWithFixedDelay(new Runnable() {
            ...
            public void run() {
                ...
                if(scheduleStatus == 0){
                    ...
                    System.out.println("当前计算尚未完成，请优化代码结构");
                }
            }
    ...
    public SnakeEntity newSnake(String accountId, String accountName, Channel channel) {
        int max = Math.min(mapHeight, mapWidth) - 10;
        int min = 10;
        Random random = new Random();
        //随机生成 出生点
        int startPoint = random.nextInt(max - min + 1) + min;
        SnakeEntity node = new SnakeEntity(this, accountId, startPoint, 3, SnakeDirection.RIGHT, channel);
